...
export default {
  ...
  props: {
    // Objet permettant de stocker les informations de la voiture courante
    voiture: {
      type: Object
    },
    ...
    // Objet permettant la communication avec le serveur via Websocket
    websocket: {
      type: WebSocket
    }
  },
  watch: {
    ...
    voiture: function (v) {
      ...
      switch (v.mode) {
        ...
        case 'DECONNEXION':
          // On réinitialise les informations de la voiture
    ...
    state: function (state) {
      ...
      // On affiche toutes les voitures déconnectées
      if (state === false) {
  ...
  methods: {
    // Cette fonction permet de faire avancer la voiture à la position donnée
    mouvementVoiture (v) {
      let voiture = this.recuperationVoiture(v.id)

      // Si on n'est pas en mode course on ne fait rien
      if (voiture.etat !== 2) return

      // On assigne les informations données par le serveur
      ...
      // On récupère une position entre 0 et 1,
      // on veut une position angulaire en radian correspondant à un angle(valeur entre 0 et 2pi)
      // On convertit notre position en radian en la multipliant par 2pi:
      // position en radian = 2 * pi * position
      // Ensuite on veut connaitre les coordonnées de notre voiture sur le circuit qui est un cercle.
      // pour les abiscisses: x = cos(position en radian) * rayon
      // pour les ordonnées: y = sin(position en radian) * rayon
      // Ici notre rayon est de 125 pixels
    ...
    ajoutVoiture (voiture) {
      // On ajoute la voiture sur l'interface si elle est connectée
      // On passe simplement son état à 1
      this.voitures.forEach((v, index) => {
    ...
    updateVoiture (voiture) {
      this.voitures.forEach((v, index) => {
        if (voiture.id === v.id) {
          this.voitures[index].position = parseFloat(voiture.position).toFixed(
            2
          )
    ...
    // Permet de gérer le classement des voitures en mode course
    updateClassement () {
    ...
    // Supprimme une voiture de l'interface
    ...
    // Elle est utilisée pour afficher l'image des voitures, car on affiche leurs images après la compilation du template.
